...
if [ "${XDEBUG}" = 1 ]; then
  ...
  # Create the Profiler and Trace output folders
  mkdir -p /tmp/xdebug/profiler_output_dir
  mkdir -p /tmp/xdebug/trace_output_dir
  chown -R www-data:www-data /tmp/xdebug

  cp /etc/apache2/conf.d/proxy_timeout.conf.disabled /etc/apache2/conf.d/proxy_timeout.conf
else
  #
  # Disable XDebug
  #
...
if [ -d /var/www/locahost/htdocs ]; then
